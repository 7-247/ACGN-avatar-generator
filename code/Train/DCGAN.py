...
# 定义超参，但是我们写的很烂，其实有些根本不能改
class Config(object):
    data_path = './input/'
    num_workers = 8  # 没用；后来改用GPU了
    img_size = 64  # 不能改
    batch_size = 64  # 批处理数量
    max_epoch = 10  # 最大轮次
    virs = "result"
    ...
    g_every = 5  # 每1个batch训练一次生成模型（必须大于d_every）
    save_every = 5  # 每save_every次保存一次模型

    netd_path = './output/netd_294.pth'  # 从哪个模型存档开始训练，置空则训练新模型
    netg_path = './output/netg_294.pth'  # 同上

    gen_img = "result.png"  # 训练完后图像会在同目录下
    gen_num = 64  # 挑选出最好64张展示
    gen_search_num = 512  # 生成512组噪点
    gen_mean = 0  # 生成模型的噪声均值，不能改
    gen_std = 1  # 噪声方差，不能改
...
class NetG(nn.Module):
    ...
    def __init__(self, opt):
        ...
        self.Gene = nn.Sequential(
            # 假定输入为1*1*opt.nz维的数据，opt.nz维的向量
            ...
            # 输入一张32*32*(ngf*2)
            nn.ConvTranspose2d(in_channels=self.ngf * 2, out_channels=3, kernel_size=4, stride=2, padding=1, bias=False),
...
class NetD(nn.Module):
    def __init__(self, opt):
        ...
        self.ndf = opt.ndf
        # DCGAN定义的判别器，生成器没有池化层。和生成器几乎对称。
        self.Discrim = nn.Sequential(
            nn.Conv2d(in_channels=3, out_channels=self.ndf * 2, kernel_size=4, stride=2, padding=1, bias=False),
...
def train(**kwargs):
    ...
    # 计算目标值和预测值之间的交叉熵损失函数
    # BCEloss:-(1/N)(ylog x +(1 - y)log(1 - x))
    ...
    for epoch in range(opt.max_epoch):
        ...
        real_image_loss.append(error_d_real.item())
        fake_image_loss.append(error_d_fake.item())
        generate_loss.append(error_g.item())
        print("轮次：", epoch)
        print("错误照片损失值：", fake_image_loss)
        print("正确照片损失值：", real_image_loss)
        print("生成器损失值：", generate_loss)
...
def generate(**kwargs):
    ...
    netd.load_state_dict(torch.load(opt.save_path + 'netd_{0}.pth'.format(opt.max_epoch - 1), map_location=map_location), False)
    netg.load_state_dict(torch.load(opt.save_path + 'netg_{0}.pth'.format(opt.max_epoch - 1), map_location=map_location), False)
    netd.to(device)
    netg.to(device)
